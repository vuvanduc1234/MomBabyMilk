...
const createManualVoucher = async (req, res) => {
  const { code, discountPercentage, description, expiryDate, minOrderValue } =
    req.body;

  if (!code || !discountPercentage || !expiryDate) {
    return res
      .status(400)
      .json({ message: "Vui lòng nhập mã, % giảm giá và ngày hết hạn" });
  }
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Tạo voucher thành công", data: newVoucher });
  } catch (err) {
...
const createRandomVoucher = async (req, res) => {
  const { discountPercentage, description, expiryDate, minOrderValue } =
    req.body;

  if (!discountPercentage || !expiryDate) {
    return res
      .status(400)
      .json({ message: "Vui lòng nhập % giảm giá và ngày hết hạn" });
  }
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Tạo voucher random thành công", data: newVoucher });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};
...
const assignVoucherToUser = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: `Đã gửi ${qtyToAdd} voucher cho user thành công` });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};
...
const applyVoucher = async (req, res) => {
  ...
  if (!code || !orderTotal) {
    return res
      .status(400)
      .json({ message: "Thiếu mã voucher hoặc tổng tiền đơn hàng" });
  }
...
const deleteVoucher = async (req, res) => {
  ...
  try {
    ...
    // VALIDATION: Kiểm tra có orders đang sử dụng voucher này không
    const ordersUsingVoucher = await OrderModel.countDocuments({
      voucherUsed: id,
    });

    if (ordersUsingVoucher > 0) {
      return res.status(400).json({
        message: `Không thể xóa voucher đang được sử dụng trong ${ordersUsingVoucher} đơn hàng`,
        ordersCount: ordersUsingVoucher,
      });
    }

    // CASCADE: Xóa voucher khỏi tất cả User.userVouchers
    await UserModel.updateMany(
      { "userVouchers.voucherId": id },
      { $pull: { userVouchers: { voucherId: id } } },
    );
    ...
    res.status(200).json({
      message: "Xóa voucher thành công (đã cleanup user references)",
    });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
