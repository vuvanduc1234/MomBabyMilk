...
const getAvailableRewards = async (req, res) => {
  try {
    ...
    const rewards = await RewardItem.find(filter)
      .populate('voucherId', 'code discountType discountValue minOrderValue')
      .sort({ pointsCost: 1 })
      .lean();
...
const createRewardItem = async (req, res) => {
  try {
    ...
    await rewardItem.populate([
      { path: 'voucherId', select: 'code discountType discountValue minOrderValue' },
    ]);
...
const updateRewardItem = async (req, res) => {
  try {
    ...
    const rewardItem = await RewardItem.findByIdAndUpdate(
      ...
      { path: 'voucherId', select: 'code discountType discountValue' },
    ]);
