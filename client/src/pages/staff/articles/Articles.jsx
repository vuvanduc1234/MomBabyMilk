...
export default function StaffArticles() {
  ...
  const pageSize = 10;
  const categories = []; // TODO: Fetch categories from API
  ...
  const filterArticles = () => {
    ...
    // Category filter (when implemented)
    if (categoryFilter !== "all") {
      filtered = filtered.filter(
        (article) => article.category?._id === categoryFilter,
      );
    }

    // Status filter (when implemented)
    if (statusFilter !== "all") {
